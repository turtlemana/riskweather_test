...
const {data, setSize, size}=useSWRInfinite((pageIndex, previousPageData) => {
    if (pageIndex === 0) return `/api/assetList?&page=1&limit=10`;
    if (pageIndex + 1 > +previousPageData.pages) return null;
    return `/api/assetList?&page=${pageIndex+1 }&limit=10`;
  },fetcher)
...
const miniChartData=useMemo(()=>{
  ...
  return (
    <div>
        ...
        <div className={'flex'}>
        <button className="btn-primary" onClick={()=>{router.push(`/explores/All-sunny`); }} >Sunny</button>
        <button className="btn-primary" onClick={()=>{router.push(`/explores/All-partly_cloud`); }}>Cloudy</button>
        <button className="btn-primary" onClick={()=>{router.push(`/explores/All-rainy`); }}>Rainy</button>
        <button className="btn-primary" onClick={()=>{router.push(`/explores/All-snowy`); }}>Snowy</button>
        </div>
